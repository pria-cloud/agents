...
const mockSendIntent = jest.fn(async (msg) => {
  if (msg.intent === 'schema.synthesise') return { ok: true, schema: 'sharedModels' };
  if (msg.intent === 'workflow.compose') return { ok: true, workflow: 'sharedWorkflows' };
  if (msg.intent === 'app.preview') return { ok: true };
  return { ok: true };
});
...
jest.mock('./a2aClient', () => ({
  sendIntent: function (...args: any[]): any { return mockSendIntent.apply(this, args); },
...
describe('handleAppComposeIntent', () => {
  ...
  it('blocks on compliance/DLP failure', async () => {
    ...
    original.handleAppComposeIntent = async function(payload: any, trace_id: any, jwt: any, parentSpan: any) {
