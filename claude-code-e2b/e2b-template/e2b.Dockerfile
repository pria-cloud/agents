...
# Copy package files for all applications
...
# Install dependencies for baseline project (development interface)
WORKDIR /code/baseline-project
RUN npm install --legacy-peer-deps

# Install dependencies for user app
WORKDIR /code/user-app
RUN npm install --legacy-peer-deps

# Install dependencies for API service
WORKDIR /code/api-service
RUN npm install
