...
function loadEnvFile() {
  ...
  if (fs.existsSync(envPath)) {
    ...
    envContent.split('\n').forEach((line, index) => {
      ...
      if (line && !line.startsWith('#')) {
        // More flexible regex to match key=value patterns
        ...
        if (match) {
          ...
          loadedCount++;
          console.log(`Line ${index + 1}: ${key}=${value ? '[present]' : '[empty]'}`);
        } else {
          console.log(`Line ${index + 1}: No match for: "${line}"`);
        }
      }
    });
  ...
  console.log('E2B_API_KEY after loading:', process.env.E2B_API_KEY ? 'present' : 'missing');
  console.log('E2B_TEMPLATE_ID after loading:', process.env.E2B_TEMPLATE_ID ? 'present' : 'missing');
}
...
async function testFinalCLIWorkaround() {
  ...
  try {
    ...
    try {
      // Use E2B SDK to connect (this should work)
