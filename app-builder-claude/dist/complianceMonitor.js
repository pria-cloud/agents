...
class ComplianceMonitor {
     ...
     */
    async checkConversationCompliance(conversationContent, files = [], context = {}) {
     ...
     */
    async runSingleCheck(check, conversationContent, files, context) {
     ...
     */
    checkTenantIsolation(check, files) {
        const violations = [];
        files.forEach(file => {
     ...
     */
    checkUserRegistration(check, files) {
        let hasSignUp = false;
     ...
     */
    checkNoHardcodedSecrets(check, files) {
        const violations = [];
     ...
     */
    checkAccessControl(check, files) {
        // This is a simplified check - in practice, you'd want more sophisticated analysis
        const hasAuth = files.some(file => file.content && (file.content.includes('auth.getUser()') ||
     ...
     */
    checkNextJSAppRouter(check, files) {
        const hasAppRouter = files.some(file => file.filePath.startsWith('app/') &&
     ...
     */
    checkTypeScriptUsage(check, files) {
        const nonTSFiles = files.filter(file => !file.filePath.endsWith('.ts') &&
     ...
     */
    checkSupabasePatterns(check, files) {
        const supabaseFiles = files.filter(file => file.content && file.content.includes('supabase'));
        ...
        const hasProperImports = supabaseFiles.every(file => file.content.includes('@/lib/supabase/') ||
     ...
     */
    checkImportAliases(check, files) {
        const violations = [];
        files.forEach(file => {
     ...
     */
    checkFileStructure(check, files) {
        const violations = [];
        files.forEach(file => {
     ...
     */
    checkProductionReady(check, files) {
        const violations = [];
        const patterns = [
            /TODO/i,
            /FIXME/i,
     ...
     */
    checkNoConsoleLogs(check, files) {
        const violations = [];
        files.forEach(file => {
     ...
     */
    checkCompleteImplementations(check, files) {
        const violations = [];
        files.forEach(file => {
            if (file.content) {
     ...
     */
    checkProperErrorHandling(check, files) {
        // This is a simplified check - you might want more sophisticated analysis
        const hasErrorHandling = files.some(file => file.content && (file.content.includes('try {') ||
     ...
     */
    checkTypeScriptTyping(check, files) {
        const violations = [];
        files.forEach(file => {
