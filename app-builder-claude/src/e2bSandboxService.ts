...
export class E2BSandboxService {
  ...
  /**
   * Attempts to automatically fix compilation errors using Claude
   */
  private async attemptErrorFix(sandbox: Sandbox, errors: string[], errorContext: any): Promise<boolean> {
    ...
    try {
      ...
      // Generate fix using Claude (simplified for now - would need proper integration)
...
AUTOFIX_EOF`, {
  ...
  /**
   * Generates error fix using Claude API for complex syntax/logic errors
   */
  private async generateErrorFix(filePath: string, content: string, errorContext: any): Promise<string | null> {
    ...
    // For complex errors, use Claude API
    try {
      logger.info({ 
        event: 'e2b.fix.claude_api_attempt', 
        file: filePath,
        errorType: type,
        lineNumber
      }, 'Attempting Claude API fix for complex error')

      const claudeClient = await this.getClaudeClient()
      if (!claudeClient) {
        logger.warn({ 
          event: 'e2b.fix.no_claude_client', 
          file: filePath 
        }, 'Claude client not available, skipping AI fix')
        return null
      }

      const fixPrompt = this.buildErrorFixPrompt(filePath, content, errorContext)
      const response = await claudeClient.query(fixPrompt, `sandbox-fix-${Date.now()}`)

      if (!response.content) {
        logger.warn({ 
          event: 'e2b.fix.claude_empty_response', 
          file: filePath 
      ...
      const fixedContent = this.extractFixedCode(response.content, content)
      if (!fixedContent) {
        logger.warn({ 
          event: 'e2b.fix.claude_parse_failed', 
          file: filePath 
        }, 'Failed to parse fixed code from Claude response')
        return null
      }

      logger.info({ 
        event: 'e2b.fix.claude_success', 
        file: filePath,
        responseLength: response.content.length
      }, 'Claude successfully generated error fix')

      return fixedContent

    } catch (error) {
      logger.error({ 
        event: 'e2b.fix.claude_error', 
        error: error instanceof Error ? error.message : String(error),
        file: filePath,
        errorType: type
      }, 'Claude API fix attempt failed')
      return null
    }
  }

  /**
   * Attempts basic pattern-based fixes before using Claude API
   */
  ...
  /**
   * Gets Claude client instance for API calls
   */
  private async getClaudeClient() {
    try {
      const { ClaudeCodeClient } = await import('./claudeCodeClient')
      return new ClaudeCodeClient({
        systemPrompt: 'You are an expert TypeScript/React developer specializing in fixing compilation errors. Provide only the corrected code without explanations.'
      })
    } catch (error) {
      logger.error({ 
        event: 'e2b.fix.claude_client_error', 
        error: error instanceof Error ? error.message : String(error) 
  ...
  /**
   * Builds a comprehensive prompt for Claude to fix the error
   */
...
**Fixed Code:**`
  ...
  /**
   * Gets error-specific instructions for Claude
   */
  ...
  /**
   * Extracts the fixed code from Claude's response
   */
