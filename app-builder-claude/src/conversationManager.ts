import { ClaudeCodeClient, ClaudeCodeResponse } from './claudeCodeClient';
...
export class ConversationManager {
  ...
  async processAppCompose(
    ...
    if (!context) {
      ...
      if (appSpec) context.appSpec = appSpec;
   ...
   */
  private async handleConversationalFlow(context: ConversationContext): Promise<ConversationResult> {
   ...
   */
  private buildAdaptiveSystemPrompt(context: ConversationContext): string {
...
Remember to follow PRIA guidelines and maintain the conversational flow. Use your tools when appropriate to read, write, or edit code.`;
   ...
   */
  private async analyzeConversationResponse(
    ...
    if (response.toolUse && response.toolUse.length > 0) {
      const fileOperations = response.toolUse.filter(tool => 
        ['write_file', 'edit_file', 'read_file'].includes(tool.name)
      );
   ...
   */
  private extractFilesFromToolUse(toolUse: any[]): any[] {
    const files: any[] = [];
   ...
   */
  private extractTechnicalDecisionsFromResponse(content: string): any {
    const decisions = {
      timestamp: new Date().toISOString(),
      extracted: content.substring(0, 500), // First 500 chars for context
    };
   ...
   */
  private getPRIAArchitectureGuidelines(): string {
...
6. **RLS Enforcement**: Every table must have Row Level Security enabled`;
   ...
   */
  private getQualityGuidelines(): string {
